import type { CarouselInternalState, CarouselProps, StateCallBack, SkipCallbackOptions } from './types';
...
const Dots = ({ props, state, goToSlide, getState }: DotsTypes): React.ReactElement<any> | null => {
	...
	const nextSlidesTable = getLookupTableForNextSlides(numberOfDotsToShow, state, props, childrenArr);
			...
			{Array(numberOfDotsToShow)
				...
				.map((_, index: number) => {
					...
					return (
						...
						>
							<button aria-label={`Go to slide ${index + 1}`} onClick={() => goToSlide(nextSlide)} />
						</li>
					);
				})}
		</ul>
