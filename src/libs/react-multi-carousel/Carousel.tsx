...
class Carousel extends Component<CarouselProps, CarouselInternalState> {
	...
	public resetTotalItems(): void {
		const totalItems = Children.count(this.props.children);
  ...
  */
	...
	public setContainerAndItemWidth(
		...
		if (this.containerRef && this.containerRef.current) {
			...
			const itemWidth: number = getItemClientSideWidth(this.props, slidesToShow, containerWidth);
			this.setState(
				{
					containerWidth,
					itemWidth,
    ...
    */
	...
	public renderLeftArrow(): React.ReactNode {
		...
		return <LeftArrow customLeftArrow={customLeftArrow} getState={() => this.getState()} previous={this.previous} />;
	}

	public renderRightArrow(): React.ReactNode {
		const { customRightArrow } = this.props;
		return <RightArrow customRightArrow={customRightArrow} getState={() => this.getState()} next={this.next} />;
	}

	public renderButtonGroups(): React.ReactElement<any> | null {
		const { customButtonGroup } = this.props;
		if (customButtonGroup) {
			return cloneElement(customButtonGroup, {
	...
	public renderDotsList(): React.ReactElement<any> | null {
		return <Dots state={this.state} props={this.props} goToSlide={this.goToSlide} getState={() => this.getState()} />;
	}
	...
	public render(): React.ReactNode {
		...
		return (
			<>
				<div className={`react-multi-carousel-list ${containerClass} ${className}`} ref={this.containerRef}>
					<ul
						ref={this.listRef}
